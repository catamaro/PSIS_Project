...
void ManageFruits(){
	...
	if(num_fruits == (num_players-1)*2) return;
...
void checkRulesMonster(struct player* dealer, struct player* receiver, int x_new, int y_new){
	...
	if(board[x_new][y_new] == EMPTY){
		...
		paint_monster(x_new, y_new , dealer->p_color->r, dealer->p_color->g, dealer->p_color->b);
	}
	// Monster moving to pacman
		...
		if(dealer->id == receiver->id)
		{
			...
			paint_pacman(old_x2, old_y2 , dealer->p_color->r,dealer->p_color->g,dealer->p_color->b);
			paint_monster(old_x1, old_y1, dealer->p_color->r,dealer->p_color->g,dealer->p_color->b);
		}
		...
		else
		{
			...
			paint_pacman(old_x1, old_y1, receiver->p_color->r,receiver->p_color->g,receiver->p_color->b);
		}
	}
	// Monster moving to superpacman
		...
		if(dealer->id == receiver->id){
			...
			paint_monster(old_x2, old_y2, dealer->p_color->r,dealer->p_color->g,dealer->p_color->b);
		}
	}
	// Monster moving to another monster
		...
		paint_monster(old_x1,old_y1,dealer->p_color->r,dealer->p_color->g,dealer->p_color->b);
		paint_monster(old_x2,old_y2,receiver->p_color->r,receiver->p_color->g,receiver->p_color->b);
	}
...
void checkRulesPacman(struct player* dealer, struct player* receiver, int x_new, int y_new){
	...
	if(board[x_new][y_new] == EMPTY){
		...
		paint_pacman(x_new, y_new , dealer->p_color->r, dealer->p_color->g, dealer->p_color->b);
	}
	// Pacman moving to monster
		...
		if(dealer->id == receiver->id){
			...
			paint_monster(old_x2, old_y2 , dealer->p_color->r,dealer->p_color->g,dealer->p_color->b);
			paint_pacman(old_x1, old_y1, dealer->p_color->r,dealer->p_color->g,dealer->p_color->b);
		}
	...
	// Pacman moving to superpacman
		...
		receiver->pacman->y = old_y2;
		paint_pacman(old_x1,old_y1,dealer->p_color->r,dealer->p_color->g,dealer->p_color->b);
		paint_powerpacman(old_x2,old_y2,receiver->p_color->r,receiver->p_color->g,receiver->p_color->b);
	}
	// Pacman moving to another pacman
		...
		receiver->pacman->y = old_y2;
		paint_pacman(old_x1,old_y1,dealer->p_color->r,dealer->p_color->g,dealer->p_color->b);
		paint_pacman(old_x2,old_y2,receiver->p_color->r,receiver->p_color->g,receiver->p_color->b);
	}
	// Pacman moving to fruit
		...
		paint_powerpacman(x_new, y_new , dealer->p_color->r,dealer->p_color->g,dealer->p_color->b);
	}
}
...
void checkRulesSuperPacman(struct player* dealer, struct player* receiver, int x_new, int y_new){
	...
	// Superpacman moving to monster
		...
		if(dealer->id == receiver->id){
			...
			paint_monster(old_x2, old_y2 , dealer->p_color->r,dealer->p_color->g,dealer->p_color->b);
			paint_powerpacman(old_x1, old_y1, dealer->p_color->r,dealer->p_color->g,dealer->p_color->b);
		}
		// Another person's monster
		else{
			int old_x1 = receiver->monster->x;
			...
			if(dealer->times < 1){
					...
					clear_place(old_x2,old_y2);
					board[old_x2][old_y2] = SUPERPACMAN;
					paint_powerpacman(old_x2,old_y2, dealer->p_color->r,dealer->p_color->g,dealer->p_color->b);
			}
			// Clean old monster coordinates
